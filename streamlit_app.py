...
# ======================================================
# IEC 60891 Rs (multi‑IV) — Procedures 1, 2, 3 (Ed.3)
# ======================================================
st.header("IEC 60891 Rs (multi‑IV)")
...
st.markdown(
    "Upload a CSV with **multiple measured light I–V curves** and their conditions. "
    "Required columns: `SweepID, V_V, I_A, G_Wm2, T_C`. "
    "Select **Procedure 1/2/3** per IEC 60891 (Ed.3, 2021). "
    "The app translates each curve to **target (G2, T2)** and estimates a **global Rs** "
    "by minimizing cross‑curve mismatch at the target."
)
...
# --- Procedure choice
proc = st.radio("IEC 60891 procedure", ["Procedure 1", "Procedure 2", "Procedure 3"], index=0, horizontal=True)
...
if proc == "Procedure 2":
    ...
    with colP21:
        kappa_init = st.number_input("Initial κ (V/A/°C)", value=0.0, step=0.001, format="%.4f",
                                     help="Curve correction factor; set 0 if unknown.")
    with colP22:
        a_voc_lnG = st.number_input("γ (V) — Voc·ln(G) term", value=0.0, step=0.005, format="%.3f",
                                    help="Non-linear voltage term: ΔV_g ≈ γ·ln(G₂/G₁). See TÜV/JRC analyses.")
    with colP23:
        fit_gamma = st.checkbox("Fit γ from data", value=False)
...
# Proc-3 knobs (interpolation)
if proc == "Procedure 3":
    samp_kind = st.selectbox("Proc‑3 interpolation axis", ["Fixed‑V (recommended)", "Fixed‑I"], index=0)
    same_T_tol = st.number_input("Max |T−T₂| for bracketing (°C)", value=2.0, step=0.5,
                                 help="Use curves within this ΔT of T₂ for G‑bracketing; otherwise fallback to nearest T.")
...
def _translate_proc1(df_sweep, Rs, kappa, alpha, beta, G2, T2):
    """IEC 60891 Procedure 1 (algebraic)."""
    V1 = df_sweep["V_V"].to_numpy(dtype=float)
    I1 = df_sweep["I_A"].to_numpy(dtype=float)
    G1 = df_sweep["G_Wm2"].to_numpy(dtype=float)
    T1 = df_sweep["T_C"].to_numpy(dtype=float)
...
def _translate_proc2(df_sweep, Rs, kappa, alpha, beta, G2, T2, gamma):
    """
    IEC 60891 Procedure 2 (algebraic with refined voltage translation).
    Adds a non-linear irradiance-dependent term on voltage (γ·ln(G2/G1)) in addition to β·ΔT.
    References discuss Proc‑2 updates and log‑G behavior near Voc in Ed.3.  # Monokroussos/TÜV; JRC 2023
    """
    V1 = df_sweep["V_V"].to_numpy(dtype=float)
    I1 = df_sweep["I_A"].to_numpy(dtype=float)
    G1 = df_sweep["G_Wm2"].to_numpy(dtype=float)
    ...
    # Current translation similar to Proc‑1 (labs widely implement this)
    Isc1 = np.nanmax(I1)
    I2 = I1 + Isc1 * (np.clip(G2, 1e-3, None) / np.clip(G1, 1e-3, None) - 1.0) + alpha * (T2 - T1)

    # Voltage translation: include Rs term + κ term + β·ΔT + γ·ln(G2/G1)
    lnG = np.log(np.clip(G2, 1e-9, None) / np.clip(G1, 1e-9, None))
    V2 = V1 - Rs * (I2 - I1) - kappa * I2 * (T2 - T1) + beta * (T2 - T1) + gamma * lnG
    return I2, V2

def _resample_fixed_V(V, I, Vgrid):
    # assumes V monotone increasing; if not, sort
    idx = np.argsort(V)
    V, I = V[idx], I[idx]
    mask = (Vgrid >= V.min()) & (Vgrid <= V.max())
    out = np.full_like(Vgrid, np.nan, dtype=float)
    if mask.any():
        out[mask] = np.interp(Vgrid[mask], V, I)
    return out

def _translate_proc3(df_sweeps, target_G, target_T, mode="fixedV", T_tol=2.0):
    """
    IEC 60891 Procedure 3 (interpolation): pick two curves around target_G at ~target_T and interpolate.
    - mode='fixedV': resample both curves on a common V-grid; linear mix on current.
    - mode='fixedI': resample on a common I-grid; linear mix on voltage.
    """
    # 1) pick a temperature slice near T2
    df = df_sweeps
    # curves within |T-T2| <= T_tol
    df_close = df[np.abs(df["T_C"] - target_T) <= T_tol].copy()
    if df_close.empty:
        # fallback: nearest T
        nearest_T = float(df.iloc[(np.abs(df["T_C"] - target_T)).idxmin()]["T_C"])
        df_close = df[np.isclose(df["T_C"], nearest_T)]
        st.warning(f"Proc‑3: no curves within ±{T_tol:.1f} °C; using nearest T = {nearest_T:.2f} °C for interpolation.")

    # 2) find bracketing curves in irradiance
    # We'll need per‑SweepID vectors
    curves = []
    for sid, g in df_close.groupby("SweepID"):
        Gi = float(g["G_Wm2"].iloc[0])
        Vi = g["V_V"].to_numpy(float)
        Ii = g["I_A"].to_numpy(float)
        curves.append((sid, Gi, Vi, Ii))

    # sort by G and bracket
    curves.sort(key=lambda x: x[1])
    Gvals = [c[1] for c in curves]
    if len(curves) < 2:
        # just return the only curve (no interpolation)
        sid, Gi, Vonly, Ionly = curves[0]
        return Ionly, Vonly

    # find index such that G_low <= target_G <= G_high
    if target_G <= Gvals[0]:
        low, high = curves[0], curves[1]
    elif target_G >= Gvals[-1]:
        low, high = curves[-2], curves[-1]
    else:
        for k in range(len(Gvals) - 1):
            if Gvals[k] <= target_G <= Gvals[k + 1]:
                low, high = curves[k], curves[k + 1]
                break

    (sidL, GL, VL, IL), (sidH, GH, VH, IH) = low, high
    w = 0.0 if GH == GL else float((target_G - GL) / (GH - GL))

    if mode == "fixedV":
        # build a common V-grid in intersection
        Vmin = max(VL.min(), VH.min())
        Vmax = min(VL.max(), VH.max())
        if Vmax <= Vmin:
            # degenerate; pick nearest single curve
            if abs(target_G - GL) <= abs(target_G - GH):
                return IL, VL
            else:
                return IH, VH
        Vgrid = np.linspace(Vmin, Vmax, 400)
        ILg = _resample_fixed_V(VL, IL, Vgrid)
        IHg = _resample_fixed_V(VH, IH, Vgrid)
        I2 = (1 - w) * ILg + w * IHg
        V2 = Vgrid
        return I2, V2
    else:
        # fixed current grid
        Imin = max(IL.min(), IH.min())
        Imax = min(IL.max(), IH.max())
        if Imax <= Imin:
            if abs(target_G - GL) <= abs(target_G - GH):
                return IL, VL
            else:
                return IH, VH
        Igrid = np.linspace(Imin, Imax, 400)
        # need V(I); invert by interpolation
        VLg = np.interp(Igrid, IL[::-1], VL[::-1], left=np.nan, right=np.nan) if np.all(np.diff(IL) < 0) else np.interp(Igrid, IL, VL)
        VHg = np.interp(Igrid, IH[::-1], VH[::-1], left=np.nan, right=np.nan) if np.all(np.diff(IH) < 0) else np.interp(Igrid, IH, VH)
        V2 = (1 - w) * VLg + w * VHg
        I2 = Igrid
        return I2, V2
...
if run_60891:
    ...
    for gid, g in groups:
        ...
        g.sort_values(by=["I_A", "V_V"], inplace=True)
        sweeps.append((gid, g))
    if len(sweeps) < 2 and proc != "Procedure 3":
        st.error("Need at least 2 measured I–V curves (preferably ≥3) for a robust 60891 Rs.")
        st.stop()
    ...
    Rs0 = float(Rs_fit) if "Rs_fit" in globals() else 0.15
    kap0 = float(kappa_init)
    gam0 = float(a_voc_lnG)

    # Build preliminary I range (for loss on a common current grid). For Proc‑3, we will do it once per sweep.
    I2_ranges = []
    for _, g in sweeps:
        if proc == "Procedure 1":
            I2p, _ = _translate_proc1(g, Rs0, kap0, alpha_Isc, beta_Voc, G2, T2)
        elif proc == "Procedure 2":
            I2p, _ = _translate_proc2(g, Rs0, kap0, alpha_Isc, beta_Voc, G2, T2, gam0)
        else:
            # For Proc‑3, get per‑sweep translation by interpolating vs whole dataset (needs dff)
            I2p, _ = _translate_proc3(dff, G2, T2, mode=("fixedV" if "Fixed‑V" in samp_kind else "Fixed‑I"), T_tol=same_T_tol)
        I2_ranges.append((np.nanmin(I2p), np.nanmax(I2p)))
    I_min = np.max([a for a, _ in I2_ranges])
    I_max = np.min([b for _, b in I2_ranges])
    if not np.isfinite(I_min) or not np.isfinite(I_max) or I_max <= I_min:
        st.error("Translated curves have no common current range — adjust target or parameters.")
        st.stop()
    I_grid = np.linspace(I_min, I_max, int(I_grid_pts))

    # Loss function
    def loss_params(Rs, kappa, gamma):
        if Rs < 0 or not np.isfinite(Rs) or not np.isfinite(kappa) or not np.isfinite(gamma):
            return 1e99
        V_stack = []
        for _, g in sweeps:
            if proc == "Procedure 1":
                ...
                I2, V2 = _translate_proc2(g, Rs, kappa, alpha_Isc, beta_Voc, G2, T2, gamma)
            else:
                # Proc‑3 ignores Rs, κ, γ; Rs estimation will be weakly sensitive; still run to build stack
                I2, V2 = _translate_proc3(dff, G2, T2, mode=("fixedV" if "Fixed‑V" in samp_kind else "Fixed‑I"), T_tol=same_T_tol)
            Vg = _interp_to_grid(I2, V2, I_grid)
            V_stack.append(Vg)
    ...
    rng = np.random.default_rng(2026)
    best = (Rs0, kap0, gam0)
    bestL = loss_params(*best)

    trials = 400
    for _ in range(trials):
        t_Rs = max(0.0, best[0] + rng.normal(0, 0.02))
        if proc == "Procedure 1":
            t_k = best[1] + (rng.normal(0, 0.002) if fit_kappa else 0.0)
            t_g = 0.0
        elif proc == "Procedure 2":
            t_k = best[1] + (rng.normal(0, 0.002) if fit_kappa else 0.0)
            t_g = best[2] + (rng.normal(0, 0.01) if fit_gamma else 0.0)
        else:
            # Proc‑3: no κ, γ effect; keep them at zero
            t_k, t_g = 0.0, 0.0
        L = loss_params(t_Rs, t_k, t_g)
        if np.isfinite(L) and L < bestL:
            best, bestL = (t_Rs, t_k, t_g), L

    Rs_iec, kappa_iec, gamma_iec = best
    msg = f"**IEC 60891 estimate ({proc}):**  Rs ≈ {Rs_iec:.4f} Ω"
    if proc in ("Procedure 1", "Procedure 2") and fit_kappa:
        msg += f",  κ ≈ {kappa_iec:.4f} V/A/°C"
    if proc == "Procedure 2" and fit_gamma:
        msg += f",  γ ≈ {gamma_iec:.4f} V"
    st.success(msg)
    ...
    for i, (gid, g) in enumerate(sweeps):
        if proc == "Procedure 1":
            ...
            I2, V2 = _translate_proc2(g, Rs_iec, kappa_iec, alpha_Isc, beta_Voc, G2, T2, gamma_iec)
        else:
            I2, V2 = _translate_proc3(dff, G2, T2, mode=("fixedV" if "Fixed‑V" in samp_kind else "Fixed‑I"), T_tol=same_T_tol)
        axC.plot(V2, I2, lw=2, color=colors[i % len(colors)], label=f"{gid}")
    axC.set_xlabel("Voltage V₂ (V) @ target")
    axC.set_ylabel("Current I₂ (A) @ target")
    axC.set_title(f"IEC 60891 {proc} → (G₂={G2:.0f} W/m², T₂={T2:.1f} °C)")
    axC.grid(alpha=0.3)
    axC.legend(ncol=2, fontsize=9)
    st.pyplot(figC)
    ...
    st.download_button("Download translated curves (CSV)",
                       df_out.to_csv(index=False).encode("utf-8"),
                       file_name=f"IEC60891_{proc.replace(' ','')}_translated_IV.csv",
                       mime="text/csv")

    st.caption(
        "Proc‑1/2 are algebraic translations; Proc‑2 refines the voltage term (e.g., non‑linear irradiance dependence). "
        "Proc‑3 is interpolation between measured curves near (G₂,T₂), and is most accurate for small–medium gaps. "
        "See IEC 60891:2021 (Ed.3) and comparative studies by JRC/TÜV/academia."
    )
